...
class TestNormallyBang < Test::Unit::TestCase
  ...
  def test_normally_down_bang_creates_down
    ...
    assert File.zero?(down_path), "down file should be empty"
  ...
  def test_normally_down_bang_with_mode
    FileUtils.rm_f(down_path)
    old_umask = File.umask(0)
    begin
      ret = @svdir.normally_down!(mode: 0644)
      assert_equal true, ret

      assert File.exist?(down_path), "down file should exist"
      stat = File.stat(down_path)
      assert_equal 0100644, stat.mode, "mode should be 0644 (file)"
    ensure
